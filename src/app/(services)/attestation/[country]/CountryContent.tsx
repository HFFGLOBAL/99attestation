...
export default function CountryContent({ countryName, documents, attestationTypes, testimonials, whatsappUrl }: CountryContentProps) {
  return (
    <main className="service-page-container">
      ...
      <section className="hero-section flex flex-col items-center justify-center text-center py-10">
        <img src="/globe.svg" alt="Attestation Globe" className="w-20 h-20 mb-4 animate-scale-in" />
        <h1 className="text-4xl md:text-5xl font-extrabold text-navy uppercase mb-2">{attestationTypes.length > 0 ? `${attestationTypes[0]} for ${countryName}` : `${countryName} Attestation`}</h1>
        <p className="text-lg text-gray-700 mb-2">Get your documents attested for use in {countryName}. Fast, secure, and globally accepted.</p>
      ...
      <section className="services-section">
        <SectionHeader title={`Documents We Attest For ${countryName.toUpperCase()}`} />
      ...
      <section className="services-section">
        <SectionHeader title={`Types Of Attestation For ${countryName.toUpperCase()}`} />
        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-6">
      ...
      <section className="how-it-works-section">
        ...
        <ol className="how-it-works-list mt-6">
          ...
          <li className="flex items-center gap-2 mb-2"><span className="bg-navy text-yellow rounded-full p-2"><svg width="18" height="18" fill="none"><polygon points="9,3 15,15 3,15" stroke="#fcbf49" strokeWidth="2" /></svg></span>Receive attested documents quickly and securely at your doorstep</li>
        </ol>
      </section>
      ...
      <section className="faq-preview-section">
        ...
        <ul style={{maxWidth: 700, margin: '0 auto', textAlign: 'left', color: '#444'}}>
          <li style={{marginBottom: '1.2rem'}}>
            ...
            <span>A: Attestation is the process of authenticating documents for use in another country.</span>
          </li>
          <li style={{marginBottom: '1.2rem'}}>
